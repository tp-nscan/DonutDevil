...
module Glauber = 
    ...
    let IntMult (intFact:int) (rhs:float) =
        rhs * System.Convert.ToDouble(intFact)

    let IntMultA (intFact:int) (rhs:float) (a:float) =
        a + rhs * System.Convert.ToDouble(intFact)

    let Glauber1D dist freq decay =
      System.Math.Exp(-decay*dist)*System.Math.Cos(freq*dist)

    let GlauberVals (range:int) freq decay =
        let rS = System.Convert.ToDouble (range-1)
