...
module NodeGroupBuilders =
    ...
    let PerturbNg1D (seed:int) (vs1D:Vs1D) (devMag:float32) (values:float32[]) =
        ...
        | UnitUnsigned ->  (PerturbInRangeF32A seed 0.0f 1.0f devMag values)
        | UnitSigned ->  (PerturbInRangeF32A seed -1.0f 1.0f devMag values)
        | UnSigned max ->  (PerturbInRangeF32A seed 0.0f max devMag values)
        | Signed absMax ->  (PerturbInRangeF32A seed -absMax absMax devMag values)
        | Ring ->  (PerturbInRangeF32A seed 0.0f 1.0f devMag values)
    ...
    let RandomNg1D (seed:int) (vs1D:Vs1D) (groupShape:GroupShape) =
        ...
        match vs1D with        
        | UnsignedBit -> myNg1D true true 1.0f
        | SignedBit -> myNg1D true false 1.0f
        | UnitUnsigned -> myNg1D false true 1.0f
        | UnitSigned -> myNg1D false false 1.0f
        | UnSigned max -> myNg1D false true max
        | Signed max -> myNg1D false false max
        | Ring -> myNg1D false true 1.0f
    ...
    let RandomNg2D (seed:int) (vs2D:Vs2D) (groupShape:GroupShape) =
        ...
        let myNodes = 
            ...
            | Complex max  -> RandDiscPointsF32 seed 1.0f |> Seq.take nodeCount |> Seq.toArray
